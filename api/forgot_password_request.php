...
try {
    ...
    $mailSent = send_password_reset_code($user['email'], $displayName, $code);
    if (!$mailSent) {
        error_log('forgot_password_request: Failed to send password reset email');
        json_out([
            'ok' => false,
            'sent' => false,
            'error' => 'Unable to send reset code right now. Please try again shortly.',
        ]);
    }
