...
function workflow(PDO $pdo): array { $rows=$pdo->query("SELECT setting_key,setting_value FROM whatsapp_bridge_settings WHERE setting_key LIKE 'trigger_%' OR setting_key LIKE 'condition_%' OR setting_key LIKE 'action_%'")->fetchAll(PDO::FETCH_KEY_PAIR); $defaults=['trigger_mention'=>1,'trigger_all_group'=>0,'trigger_confirm'=>1,'condition_format'=>1,'condition_confirm'=>1,'action_private_dm'=>1,'action_queue'=>1,'action_post_group'=>1,'action_create_ride'=>1]; foreach($defaults as $k=>$v)$defaults[$k]=($rows[$k]??(string)$v)==='1'; return $defaults; }
