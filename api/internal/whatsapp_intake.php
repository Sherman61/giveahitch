...
function workflow(PDO $pdo): array { $rows=$pdo->query("SELECT setting_key,setting_value FROM whatsapp_bridge_settings WHERE setting_key LIKE 'trigger_%' OR setting_key LIKE 'condition_%' OR setting_key LIKE 'action_%' OR setting_key LIKE 'keyword_%' OR setting_key='format_help'")->fetchAll(PDO::FETCH_KEY_PAIR); $defaults=['trigger_mention'=>1,'trigger_all_group'=>0,'trigger_confirm'=>1,'condition_format'=>1,'condition_confirm'=>1,'action_private_dm'=>1,'action_queue'=>1,'action_post_group'=>1,'action_create_ride'=>1]; foreach($defaults as $k=>$v)$defaults[$k]=($rows[$k]??(string)$v)==='1'; $defaults['keywords']=['looking'=>$rows['keyword_looking']??'looking,need,request','offering'=>$rows['keyword_offering']??'offering,offer,drive,available','from'=>$rows['keyword_from']??'from','to'=>$rows['keyword_to']??'to','going_to'=>$rows['keyword_going_to']??'going to,going-to','call'=>$rows['keyword_call']??'call','whatsapp'=>$rows['keyword_whatsapp']??'whatsapp','note'=>$rows['keyword_note']??'note']; $defaults['format_help']=$rows['format_help']??'@ridebot Looking from Kingston to Montego Bay call: 876-555-1234 whatsapp: 876-555-9999 note: Need to arrive before 5 PM.'; return $defaults; }
